...
4. **Настройка сайта (важно для Production)**
   ...
   - Долгий анализ в n8n: API ждёт ответ webhook до **200 секунд**; на Vercel для функции задан `maxDuration = 300` с. На **Hobby** лимит функции часто **10 с** — тогда нужен **Pro** (или укоротить workflow), иначе запрос обрежется раньше.
